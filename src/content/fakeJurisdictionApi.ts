import {Jurisdiction} from './schemas';
...
function findJurisdiction(jurisdictionId: Jurisdiction['id']) {
  return findJurisdictionRecursive(fakeJurisdictionsData.jurisdictions, jurisdictionId);
}

export async function fetchJurisdiction(jurisdictionId: Jurisdiction['id']) {
  await delay(1000);
...
export async function fetchSubJurisdictions(jurisdictionId: Jurisdiction['id']) {
  await delay(1500);
  const jurisdiction = findJurisdiction(jurisdictionId);
  ...
  return (jurisdiction.subJurisdictions || []).map((j: Jurisdiction) => ({
    id: j.id,
    name: j.name,
  }));
}
